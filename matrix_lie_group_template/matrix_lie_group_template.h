...
namespace inekf {
    ...
    // |         .          .   .          . |
    // | 0_(1 * N)          0 ...          1 |
    template <unsigned int N = 1, unsigned int K = 0>
    class SEKN {
    public:
        EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
    ...
    public:
        ...
        SEK3(Eigen::VectorXd xi);

        SEK3(Eigen::MatrixXd X);

        SEK3(const SEK3<K>& sek3);
        ...
        void setRotation(const SEK3<K>& sek3);
        ...
        void setRotationDelay(const SEK3<K>& sek3);
        ...
        void setVector(unsigned int idx, const SEK3<K>& sek3);
    ...
    public:
        ...
        // functions
        Eigen::VectorXd getTVec() const { return this->getXi().template tail<this->N_>(); }

        Eigen::MatrixXd getTMat() const { return this->getX(); }
        // get hat matrix of Lie algebra \xi
        Eigen::MatrixXd getHat() const override;
        // get the inversion of T(N)
        TN<N> getInverse() const;
        // set vector of T(N)
        void setTVec(const Eigen::VectorXd& vec);
        // set matrix of T(N)
        void setTMat(const Eigen::MatrixXd& mat);
        // override operator +
        TN<N> operator+(const TN<N>& y) const;
        // override operator -
        TN<N> operator-(const TN<N>& y) const;
        // override operator *
        TN<N> operator*(const double& k) const;
        // override operator =
        TN<N>& operator=(const TN<N>& y);
